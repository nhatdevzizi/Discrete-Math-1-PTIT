...
| `TRR1_2009.cpp` | **Tổng hàm Euler** `Σ φ(i)` với `i = 1..n`. | Tính `φ(i)` cho mọi `i` bằng **sàng tuyến tính** (mỗi hợp số chỉ bị đánh dấu đúng một lần thông qua ước nguyên tố nhỏ nhất của nó), sau đó cộng tổng mảng lại. |
